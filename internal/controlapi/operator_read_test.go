...
func TestOperatorReadStaticViews(t *testing.T) {
	a := testAPI(t)
	reader := &fakeOperatorReader{
		targets: []operatorview.TargetView{{Name: "dns01", Address: "192.0.2.53:22"}},
...
func TestOperatorReadDoesNotExposeInternalError(t *testing.T) {
	a := testAPI(t)
	reader := &fakeOperatorReader{err: errors.New("postgres://example-user:" + "example-password@db.invalid/private")}
	h := a.OperatorReadHandler(reader)
