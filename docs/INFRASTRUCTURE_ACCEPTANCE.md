...
This runbook is the repeatable core real-infrastructure acceptance procedure for the accepted `0.1.0-dev.20` baseline and the `0.1.0-dev.21` hardening candidate. It is deliberately conservative: use disposable/constrained guests, keep direct Gateway exposure private during acceptance, and do not treat a deployment/release candidate as accepted until every required positive and negative check passes.
...
target-test  disposable SSH execution target
```
...
```sh
export DB_IP='192.0.2.20'
export CONTROL_IP='192.0.2.10'
export GATEWAY_IP='192.0.2.11'
...
export UNLISTED_LAN_IP='192.0.2.99'
```
...
printf '%s\n' 'target-test' > /etc/tethys-sentinel/target-id
...
Create a minimal authoritative `/etc/tethys-sentinel/context.json` based on `config/context.example.json`, with only `target-test` in the acceptance inventory. Treat all output/history/notes as non-authoritative.
