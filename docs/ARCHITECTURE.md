...
3. UDP/123 to operator-configured trusted literal-IP NTP servers.

`sentinel-egress-policy` renders/verifies deny-by-default PVE rules, installed-policy drift, Datacenter firewall activation and the VM NIC `firewall=1` requirement. It requires at least one trusted NTP source because Worker validates short-lived SSH certificate time bounds against its local clock. It never applies policy as a Worker privilege.

Packet-level tests and verified guest clock synchronization remain mandatory for infrastructure acceptance.
...
The AI-facing inventory is additionally privacy-reduced: configured host network `addresses` are omitted from `INFRASTRUCTURE.json` even when they exist in the operator-owned context source. The privileged Operator snapshot may inspect the full configured context, but has no v1 mutation route for it.
