...
func TestSignerProducesTightlyConstrainedUserCertificate(t *testing.T) {
	...
	response, err := service.Sign(Request{
		JobID: "job-00000001", GrantID: "grant-00000001", Target: "target-a",
	...
	if got := cert.CriticalOptions["source-address"]; got != "192.0.2.50/32" {
...
func TestSignerRejectsNonEd25519EphemeralKey(t *testing.T) {
	...
	_, err = service.Sign(Request{
		JobID: "job-00000002", GrantID: "grant-00000001", Target: "target-a",
...
func TestSignerRejectsShellUnsafeIdentifiersAndWrapper(t *testing.T) {
	...
	if _, err := New(ca, Policy{
		...
		SourceAddresses: []string{"192.0.2.50"}, CertificateTTL: 45 * time.Second, Backdate: 5 * time.Second,
	}); err == nil {
	...
	_, err = service.Sign(Request{
		JobID: "job;reboot", GrantID: "grant-00000001", Target: "target-a",
...
func TestSignerRequiresExactSourceAddresses(t *testing.T) {
	...
	if _, err := New(ca, Policy{
		...
		SourceAddresses: []string{"192.0.2.0/24"}, CertificateTTL: 45 * time.Second, Backdate: 5 * time.Second,
	}); err == nil {
...
func TestSignerRejectsExpiredUpperBound(t *testing.T) {
	...
	_, err = service.Sign(Request{
		JobID: "job-00000003", GrantID: "grant-00000001", Target: "target-a",
