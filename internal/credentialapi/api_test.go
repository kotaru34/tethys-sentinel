...
func TestRunningJobReceivesJobBoundCertificateAndResolvedTarget(t *testing.T) {
	...
	if response.Target.Name != "target-a" || response.Target.Address != "192.0.2.53:22" || response.Target.User != "sentinel-ai" || response.Target.HostKey == "" {
...
func newFixtureWithCommand(t *testing.T, start bool, argv []string, permissions domain.Permissions, storedRisk *risk.Result) *testFixture {
	...
	grant, _, err := caps.Issue(context.Background(), domain.Grant{
		Agent: "agent-a", Purpose: "test SSH signer", Targets: []string{"target-a"},
	...
	job, created, err := jobs.Enqueue(context.Background(), executionjob.EnqueueInput{
		RequestID: "req-credential-01", GrantID: grant.ID, Agent: grant.Agent, Target: "target-a",
	...
	targets, err := sshtarget.NewStatic([]sshtarget.Spec{{
		Name: "target-a", Address: "192.0.2.53:22", User: "sentinel-ai", HostKey: targetHostKey(t),
	}})
	if err != nil {
		t.Fatal(err)
	}
