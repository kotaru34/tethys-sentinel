...
func TestStaticTargetStorePinsCanonicalHostKey(t *testing.T) {
	...
	store, err := NewStatic([]Spec{{Name: "target-a", Address: "192.0.2.53:22", User: "sentinel-ai", HostKey: hostKey + " comment"}})
	if err != nil {
		t.Fatal(err)
	}
	spec, err := store.Resolve("target-a")
	if err != nil {
		t.Fatal(err)
	}
	if spec.Address != "192.0.2.53:22" || spec.User != "sentinel-ai" {
...
func TestTargetStoreListIsSortedIndependentSnapshot(t *testing.T) {
	...
	store, err := NewStatic([]Spec{
		{Name: "zeta", Address: "192.0.2.54:22", User: "sentinel-ai", HostKey: key},
		{Name: "alpha", Address: "192.0.2.53:22", User: "sentinel-ai", HostKey: key},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTargetStoreRejectsUnknownFieldsAndWritableConfig(t *testing.T) {
	...
	content := `{"targets":[{"name":"target-a","address":"192.0.2.53:22","user":"sentinel-ai","host_key":"` + testHostKey(t) + `","surprise":true}]}`
	...
	content = `{"targets":[{"name":"target-a","address":"192.0.2.53:22","user":"sentinel-ai","host_key":"` + testHostKey(t) + `"}]}`
...
func TestTargetStoreRejectsMalformedOrNonGlobalAddress(t *testing.T) {
	...
	for _, address := range []string{
		"192.0.2.53",
		"target-a.internal:22",
		"0.0.0.0:22",
		"127.0.0.1:22",
		"[::1]:22",
		"[fe80::53]:22",
	} {
		if _, err := NewStatic([]Spec{{Name: "target-a", Address: address, User: "sentinel-ai", HostKey: key}}); err == nil {
			t.Fatalf("unsafe SSH target endpoint accepted: %s", address)
		}
	}
	if _, err := NewStatic([]Spec{{Name: "target-a", Address: "192.0.2.53:22", User: "-root", HostKey: key}}); err == nil {
